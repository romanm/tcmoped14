...
public class Times implements MObject, Serializable, Comparable<Times> {
    ...
    /**
     * 
     <xsl:choose> <xsl:when test="@apporder=0">..></xsl:when> Nach Ende <xsl:when test="@apporder=1">&lt;..</xsl:when>
     * Vor Beginn <xsl:when test="@apporder=3">.&lt;.</xsl:when> Endet vor Beginn --- <xsl:when
     * test="@apporder=2">.>.</xsl:when> Nach Beginn </xsl:choose>
     */
	...
	if (relunit == null)
	    hms = 1000;
	else if (relunit.equals("H"))
	    hms = 60 * 60 * 1000;
	else if (relunit.equals("M"))
	    hms = 60 * 1000;
	else if (relunit.equals("S"))
	    hms = 1000;
	return hms;
    }
	...
	if (hasAbs())
	    return 0;
	if (apporder == null && relvalue == null)
	    return 0;
    ...
    public void setRelvalue(Integer relvalue) {
	...
	return "times:" + getId() + " :abs:" + getAbs() + " :apporder:" + getApporder() + " :relunit:" + getRelunit()
		+ " :relvalue:" + getRelvalue() + " :visual:" + getVisual();
    }

    public int compareTo(MObject t) {
	return compareTo((Times) t);
    }
	...
	if (t == null)
	    return -1;
	if (hasAbs())
	    return !t.hasAbs() ? -1 : abs.compareTo(t.getAbs());
	else
	    return t.hasAbs() ? 1 : compareApporder(t);
    }
	...
	if (hasApporder())
	    ...
	    return t.hasApporder() ? 1 : compareUnit(t);
    }
	...
	if (hasUnit())
	    ...
	    return t.hasUnit() ? 1 : compareRelvalue(t);
    }
	...
	if (hasRelvalue())
	    ...
	    return t.hasRelvalue() ? 1 : compareVisual(t);
    }

    private int compareVisual(Times t) {
	if (hasVisual())
	    return !t.hasVisual() ? -1 : visual.compareTo(t.getVisual());
	else
	    return t.hasVisual() ? 1 : 0;// 0==all 0
    }

    private boolean hasUnit() {
	return relunit != null && relunit.length() > 0;
    }
    ...
    public boolean hasRelvalue() {
	return relvalue != null;
    }

    public boolean hasApporder() {
	return apporder != null && apporder.length() > 0;
    }
